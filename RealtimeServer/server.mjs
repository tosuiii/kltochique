...
app.get("/", (_req, res) => res.json({
  ...
  version: "4.2.1-cyber-session-consent"
}));
...
function summaryFor(a, admin) {
  return {
    ...
    sessionActive: a.viewPermissions.has(admin.id) && a.controlOwnerId === admin.id,
    keylogActive: a.keylogOwnerId === admin.id,
    inputLocked: !!a.inputLocked,
    maintenanceActive: !!a.maintenanceActive && a.maintenanceOwnerId === admin.id,
    maintenanceBusy: !!a.maintenanceActive && a.maintenanceOwnerId !== admin.id,
...
function syncStream(a) {
  const canStream = !a.maintenanceActive && hasActiveAuthorizedWatcher(a);
  send(a.ws, { type: canStream ? "stream_start" : "stream_stop" });
}
...
function syncPermissions(a) {
  send(a.ws, {
    ...
    sessionAuthorized: a.viewPermissions.size > 0 && !!a.controlOwnerId,
...
function clearAdminRights(a, adminId) {
  ...
  if (a.maintenanceOwnerId === adminId) {
    if (a.maintenanceActive) send(a.ws, { type: "maintenance_stop" });
    a.maintenanceOwnerId = null;
      ...
      switch (msg.type) {
        case "request_session": {
          if (a.controlOwnerId && a.controlOwnerId !== ws.id) {
            return send(ws, { type: "error", message: "Este computador já está em uma sessão de controle com outro operador." });
          }
          const requestId = newRequest(ws, a, "session");
          send(a.ws, { type: "session_request", requestId, requester: ws.label });
          return;
        }
        ...
        case "request_input_lock": {
          ...
          if (a.maintenanceActive) {
            return send(ws, { type: "error", message: "O modo de manutenção já está protegendo a tela e o input local." });
          }
          ...
          if (a.viewPermissions.has(ws.id) && a.controlOwnerId === ws.id) {
            send(a.ws, { type: "input_lock_apply", active: true });
            return;
          }
        ...
        case "request_maintenance": {
          if (!requireViewPermission(ws, a)) return;
          if (a.controlOwnerId !== ws.id) {
            return send(ws, { type: "error", message: "O modo de manutenção exige controle remoto autorizado para este operador." });
          }
          if (a.maintenanceActive || a.inputLocked) {
            return send(ws, { type: "error", message: "Já existe um bloqueio local ou modo de manutenção ativo." });
          }
          if (a.viewPermissions.has(ws.id) && a.controlOwnerId === ws.id) {
            a.maintenanceOwnerId = ws.id;
            send(a.ws, { type: "maintenance_start" });
            return;
          }
          const requestId = newRequest(ws, a, "maintenance");
          send(a.ws, { type: "maintenance_request", requestId, requester: ws.label });
          return;
        }

        case "end_maintenance": {
          if (a.maintenanceOwnerId === ws.id || a.controlOwnerId === ws.id) {
            send(a.ws, { type: "maintenance_stop" });
          }
          return;
        }
        ...
        case "shell_cmd": {
          ...
          if (a.maintenanceActive) {
            return send(ws, { type: "error", message: "Comandos ficam pausados durante o modo de manutenção com tela protegida." });
          }
        ...
        case "end_control": {
          if (a.controlOwnerId === ws.id) {
            if (a.maintenanceActive && a.maintenanceOwnerId === ws.id) send(a.ws, { type: "maintenance_stop" });
            a.maintenanceOwnerId = null;
            a.maintenanceActive = false;
        ...
        case "end_access": {
          ...
          if (a.maintenanceActive && a.maintenanceOwnerId === ws.id) send(a.ws, { type: "maintenance_stop" });
          if (a.maintenanceOwnerId === ws.id) { a.maintenanceOwnerId = null; a.maintenanceActive = false; }
    ...
    if (ws.role === "agent") {
      ...
      switch (msg.type) {
        case "session_response": {
          const hit = getPending(msg.requestId, a.id, "session");
          if (!hit) return;
          const allow = msg.allow === true && (!a.controlOwnerId || a.controlOwnerId === hit.admin.id);
          if (allow) {
            a.viewPermissions.add(hit.admin.id);
            a.controlOwnerId = hit.admin.id;
          } else {
            clearAdminRights(a, hit.admin.id);
          }
          notifyConsent(hit.admin, hit.p.requestId, a.id, "session", allow);
          syncPermissions(a);
          broadcast();
          return;
        }
        ...
        case "maintenance_response": {
          const hit = getPending(msg.requestId, a.id, "maintenance");
          if (!hit) return;
          const allow = msg.allow === true && a.controlOwnerId === hit.admin.id && a.viewPermissions.has(hit.admin.id);
          if (allow) a.maintenanceOwnerId = hit.admin.id;
          else { a.maintenanceOwnerId = null; a.maintenanceActive = false; }
          notifyConsent(hit.admin, hit.p.requestId, a.id, "maintenance", allow);
          broadcast();
          return;
        }

        case "maintenance_ack": {
          const active = msg.status === true && !!a.maintenanceOwnerId && a.controlOwnerId === a.maintenanceOwnerId;
          a.maintenanceActive = active;
          a.inputLocked = active ? true : false;
          if (!active) a.maintenanceOwnerId = null;
          const owner = [...admins].find(x => x.id === a.controlOwnerId);
          if (owner) send(owner, { type: "maintenance_ack", agentId: a.id, status: active });
          syncPermissions(a);
          broadcast();
          return;
        }
        ...
        case "maintenance_timeout": {
          ...
          a.inputLocked = false;
          const owner = [...admins].find(x => x.id === a.controlOwnerId);
          if (owner) send(owner, { type: "maintenance_timeout", agentId: a.id });
          syncPermissions(a);
          broadcast();
          return;
        }
        ...
        case "lock_ack": {
          a.inputLocked = a.maintenanceActive ? true : (msg.status === true);
          const owner = [...admins].find(x => x.id === a.controlOwnerId);
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`KL TOCHIQUE Realtime 4.2.1 listening on ${PORT}`);
});
