...
internal static class Program
{
    ...
    public class AgentState {
        public bool AccessActive = true, StreamRequested = true, ControlActive = true, SessionAuthorized = true;
    ...
    static async Task CaptureLoop(CancellationTokenSource lt, string id, ClientWebSocket s, SemaphoreSlim g, AgentState st) {
        ...
        try {
            while (!lt.IsCancellationRequested) {
                if (!st.AccessActive || !st.StreamRequested || s.State != WebSocketState.Open) { 
                    await Task.Delay(100, lt.Token); 
                    continue; 
                }
                ...
                await Task.Delay(Math.Max(1, delay), lt.Token);
            }
        } catch { }
        finally { 
            fg?.Dispose(); f?.Dispose(); sg?.Dispose(); sc?.Dispose(); ep?.Dispose(); ms?.Dispose(); 
        }
    }
