...
namespace EmpresaMonitor.Agent
{
        ...
        static async Task CaptureLoop(AgentState state)
        {
            ...
            try
            {
                while (!state.Lifetime.IsCancellationRequested)
                {
                    ...
                    // Overlay ativo → oculta-o só durante o CopyFromScreen para o operador
                    // continuar vendo a tela real (mesma lógica dos eventos injetados no
                    // LocalInputBlocker: o que vem do operador passa, o que é local é oculto).
                    var overlayActive = ScreenOverlay.IsActive;
    ...
    //   - O usuário local vê o overlay; o operador continua vendo a TELA REAL: como a
    //     captura do Agent usa GDI (CopyFromScreen), que NÃO honra
    //     WDA_EXCLUDEFROMCAPTURE, o overlay é ocultado (SW_HIDE) só durante o instante
    //     da captura pelo CaptureLoop (ScreenOverlay.SuspendForCapture/ResumeAfterCapture).
    //   - A janela é click-through (WS_EX_TRANSPARENT): cliques físicos ou injetados
    //     atravessam o overlay e chegam ao desktop.
    //   - O cursor local fica oculto enquanto o overlay estiver ativo.
    //   - SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE) permanece para capturadores
    //     modernos (Windows.Graphics.Capture, Win10 build 19041+; falha é ignorada).
    // ---------------------------------------------------------------------------
    ...
    internal static class ScreenOverlay
    {
        ...
        // Oculta o overlay apenas pelo instante da captura do frame remoto. A captura
        // via GDI (CopyFromScreen) não honra WDA_EXCLUDEFROMCAPTURE, então sem isso o
        // operador veria a tela preta/"Atualizando" em vez da tela real.
