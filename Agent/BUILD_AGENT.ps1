...
try {
    ...
    # Gera os bytes XOR da passphrase (mesmo esquema do decode no BuildConfig.cs: XOR 0x5A).
    # Nunca emitir a passphrase como literal C#: Roslyn constant-folda a concatenação
    # para ldstr e a string apareceria em texto contíguo no EXE final.
    $ppEnc = ([System.Text.Encoding]::UTF8.GetBytes($passphrase) | ForEach-Object { '0x{0:X2}' -f ($_ -bxor 0x5A) }) -join ', '
